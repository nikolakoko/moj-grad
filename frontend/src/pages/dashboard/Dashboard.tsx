    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 overflow-auto p-8">
        ...
        <div className="flex justify-center items-center gap-3 mt-6">
          ...
          <span className="text-sm">Page {currentPage} / {totalPages || 1}</span>
          <button
            className="px-4 py-2 rounded-xl border border-gray-200 bg-white text-sm font-medium hover:bg-gray-50 disabled:opacity-40 transition"
      ...
      {/* ── DETAIL DIALOG ── исто стилизирање како folder-tidy-up, без edit ── */}
      <Dialog open={!!selectedComplaint} onOpenChange={() => setSelectedComplaint(null)}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl">
          <DialogHeader>
            <DialogTitle>Детали за жалба #{sc?.id}</DialogTitle>
          </DialogHeader>
          ...
          {sc && (
            <div className="space-y-6">
              ...
              <div>
                <Label>Наслов</Label>
                <p className="text-lg font-semibold mt-1">{sc.title}</p>
              </div>
 
              {/* ОПИС */}
              <div>
                <Label>Опис</Label>
                <p className="text-gray-700 mt-1">{sc.description}</p>
              </div>
              ...
              <div>
                ...
                <div className="mt-2 p-4 bg-gray-50 rounded-lg space-y-2">
                  <p className="text-sm text-gray-700 flex items-center gap-2">
                    ...
                    {addresses[sc.id] ?? 'Вчитување...'}
                  </p>
                  <p className="text-xs text-gray-500">
                    Координати: {sc.latitude}, {sc.longitude}
                  </p>
                </div>
              </div>
 
              {/* СТАТУС / ПРИОРИТЕТ / ОДДЕЛ */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <Label>Статус</Label>
                  <div className="mt-2">
                    <span className={`inline-block px-3 py-1.5 rounded-lg text-sm font-medium border ${statusChipColors[scStatusKey] ?? 'bg-gray-100 text-gray-600'}`}>
                      {statusLabels[scStatusKey] ?? scStatusKey}
                    </span>
                  </div>
                </div>
                <div>
                  ...
                  <div className="mt-2">
                    <span className={`inline-block px-3 py-1.5 rounded-lg text-sm font-medium border ${priorityChipColors[scPriorityKey] ?? 'bg-gray-100 text-gray-600'}`}>
                      {priorityLabels[scPriorityKey] ?? scPriorityKey}
                    </span>
                  </div>
                </div>
                <div>
                  <Label>Оддел</Label>
                  <div className="mt-2">
                    <span className="inline-block px-3 py-1.5 rounded-lg text-sm font-medium bg-gray-100 text-gray-700 border border-gray-200">
                      {sc.departmentName ?? '-'}
                    </span>
                  </div>
                </div>
